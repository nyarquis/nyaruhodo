...
def read(file_path, file_type):
    ...
    if header_bytes[:3] == b"ID3":
        ...
        while frame_offset < len(id3_bytes) - (frame_id_size + frame_size_length + 2):
            ...
            frame_id = id3_bytes[frame_offset: frame_offset +
                                 frame_id_size].decode("ascii", errors="replace")
            ...
            if version_major == 2:
                ...
                frame_size = (id3_bytes[frame_offset+3] << 16) | (
                    id3_bytes[frame_offset+4] << 8) | id3_bytes[frame_offset+5]
                frame_data = id3_bytes[frame_offset +
                                       6: frame_offset + 6 + frame_size]
                ...
                if version_major == 4:
                    ...
                    frame_size = ((id3_bytes[frame_offset+4] & 0x7F) << 21 | (id3_bytes[frame_offset+5] & 0x7F) << 14 | (
                        id3_bytes[frame_offset+6] & 0x7F) << 7 | (id3_bytes[frame_offset+7] & 0x7F))

                else:

                    frame_size = struct.unpack_from(
                        ">I", id3_bytes, frame_offset + 4)[0]

                frame_data = id3_bytes[frame_offset +
                                       10: frame_offset + 10 + frame_size]
