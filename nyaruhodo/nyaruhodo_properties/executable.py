...
def read_pe(file_path):
    ...
    try:
        ...
        properties['Architecture'] = PE_MACHINES.get(machine_code, f'0x{machine_code:04X}')
        properties['CompileTime']  = pe_timestamp_to_string(timestamp)
        properties['SectionCount'] = section_count
        characteristic_labels = [label for flag, label in PE_CHARACTERISTICS.items() if characteristics_flags & flag]
        ...
        if optional_header_size >= 68:
            ...
            if subsystem_field_offset + 2 <= len(file_bytes):
                ...
                properties['Subsystem'] = PE_SUBSYSTEMS.get(subsystem_code, str(subsystem_code))
...
def read_elf(file_path):
    ...
    try:
        ...
        properties['OS/ABI']     = ELF_OSABI.get(osabi_code, f'0x{osabi_code:02X}')
        ...
        properties['Type']         = ELF_TYPES.get(elf_file_type, f'0x{elf_file_type:04X}')
        properties['Architecture'] = ELF_MACHINES.get(machine_code, f'0x{machine_code:04X}')
