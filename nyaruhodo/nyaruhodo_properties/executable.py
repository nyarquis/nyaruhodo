...
def ReadPortableExecutable(file_path):
    ...
    try:
        ...
        machine_code = struct.unpack_from(
        ...
        characteristics_flags = struct.unpack_from(
        ...
        if optional_header_size >= 68:
            ...
            optional_magic = struct.unpack_from(
                "<H", file_bytes, pe_header_offset + 24)[0]
            properties["PEFormat"] = "PE32+" if optional_magic == 0x20B else "PE32"
            subsystem_field_offset = pe_header_offset + \
                24 + (68 if optional_magic != 0x20B else 84)

            if subsystem_field_offset + 2 <= len(file_bytes):

                subsystem_code = struct.unpack_from(
                    "<H", file_bytes, subsystem_field_offset)[0]
                properties["Subsystem"] = tables.PE_SUBSYSTEMS.get(
                    subsystem_code, str(subsystem_code))
        ...
        for section_index in range(section_count):
            ...
            section_name = file_bytes[section_entry_offset: section_entry_offset + 8].rstrip(
...
def ReadExecutableAndLinkableFormat(file_path):
    ...
    try:
        ...
        properties["OS/ABI"] = tables.ELF_OSABI.get(
            osabi_code, f"0x{osabi_code:02X}")
        ...
        properties["Type"] = tables.ELF_TYPES.get(
            elf_file_type, f"0x{elf_file_type:04X}")
        properties["Architecture"] = tables.ELF_MACHINES.get(
            machine_code, f"0x{machine_code:04X}")
