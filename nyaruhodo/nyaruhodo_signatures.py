    ...
    try:
        ...
        for signature in signatures:
            ...
            try:
                ...
                print(
                    f"LOADING SIGNATURE: {signature_description}", end=" ")
                load_time = 3
                signature_bytes = bytes.fromhex(signature_hex)
                SIGNATURES[signature_bytes] = (
                    signature_file_type, signature_description)
                signatures_count += 1
                ...
                print(f"\r\033[{os.get_terminal_size().columns - len('OK') - 1}C{GREEN}OK{RESET}")
                ...
                print(
                    f"==> {YELLOW}WARNING{RESET} [{os.path.basename(__file__)}]: FAILED TO LOAD SIGNATURE ({signature_hex.upper()}): {exception_string.upper()}")
                continue

        if signatures_count:

            print(
                f"\n==> {GREEN}SUCCESS{RESET}: LOADED {signatures_count} SIGNATURE(S)\n")

        else:

            print(f"\n==> {RED}ERROR{RESET}: NO SIGNATURES WERE LOADED. VERIFY THE INTEGRITY OF SIGNATURES.JSON\n")
